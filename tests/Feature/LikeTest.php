...
use App\Models\{User, UserBan, BanReason, Forum, Category, ForumStatus, Notification, CategoryStatus, Thread, ThreadStatus, Post, PostStatus, ThreadVisibility, Like};
use Carbon\Carbon;
...
class LikeTest extends TestCase
{
    ...
    /** @test */
    public function resource_like_data_validation() {
        $user = $this->user;
        $this->actingAs($user);

        $response = $this->post('/thread/like', ['resourceid'=>847]);
        $response->assertRedirect();
        $response->assertSessionHasErrors('resourceid');

        $response = $this->post('/post/like', ['resourceid'=>847]);
        $response->assertRedirect();
        $response->assertSessionHasErrors('resourceid');
    }

    /** @test */
    public function temp_banned_user_cannot_like() {
        $user = $this->user;
        $this->actingAs($user);
        ...
        $thread = $this->thread;
        $response = $this->post('/thread/like', ['resourceid'=>$thread->id]);
        $response->assertForbidden();
    }

    /** @test */
    public function permanent_banned_user_cannot_like() {
        $user = $this->user;
        $this->actingAs($user);
    ...
    public function post_like() {
        ...
        $this->assertCount(0, Like::all());
        $post = $this->post;
        $response = $this->post('/post/like', ['resourceid'=>$post->id]);
        $this->assertCount(1, Like::all());
    }

    /** @test */
    public function cannot_like_hidden_threads() {
    ...
    /** @test */
    public function cannot_like_posts_that_belongs_to_hidden_threads() {
        $user = $this->user;
        $this->actingAs($user);
        $other = User::factory()->create(['id'=>1701, 'username'=>'hostname49']);
        
        // private thread's posts like
        $thread = Thread::create(['user_id'=>$other->id,'subject'=>'This is subject','category_id'=>Category::first()->id,'status_id'=>ThreadStatus::where('slug', 'live')->first()->id,'visibility_id'=>ThreadVisibility::where('slug', 'private')->first()->id,'type'=>'poll','replies_off'=>0]);
        $post = Post::create([
            'user_id'=>$other->id,
            'thread_id'=>$thread->id,
        ...
        // check followers-only thread's posts like
        $thread->update(['visibility_id'=>ThreadVisibility::where('slug', 'followers-only')->first()->id]);
        $response = $this->post('/post/like', ['resourceid'=>$post->id]);
        $response->assertStatus(404);
